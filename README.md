...
项目面向**全场景多智能体协同**需求：无论是运维排障、监控分析、故障根因定位，还是需要把数据检索、工具执行、知识推理与结果沉淀成报告/交付物的业务场景，BoGuan 都提供从“管理配置 → 规划执行 → 证据可追溯 → 一键导出/分发”的闭环能力。

**核心卖点（开箱即用、快速上手）：**
- **多 Agent 并行对话**：不同 Agent 会话互不干扰，可同时推进与快速切换，支持多轮上下文与历史持久化
- **计划驱动的执行可视化**：中间展示执行计划状态变化，右侧展示执行详情（思考/工具/证据），让排障过程可解释、可追溯
- **一键导出与分发**：生成根因分析 PDF，可直接下载；也可通过 SMTP 将 PDF 发送到指定邮箱
- **Skills + MCP 即插即用**：用 Markdown 技能定义工作流程，用 MCP 接入企业工具与数据源，并支持配置热更新
...
- **多 Agent 同时对话**：不同 Agent 的对话互不干扰，可在 Web UI 中并行推进与快速切换，适合同时排查多个告警/问题
